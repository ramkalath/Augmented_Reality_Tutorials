...
if __name__ == "__main__":
    ...
    col0 = np.tile(np.arange(0, cols), rows)
    col1 = np.reshape((np.ones((cols, rows))* np.arange(rows)).T, (1, cols*rows ))[0]
    col2 = np.zeros(rows*cols)
    objpts = np.vstack([col0, col1, col2]).T.reshape(rows*cols, 3)
